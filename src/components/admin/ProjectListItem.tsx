...
export default function ProjectListItem({ project, onAssign, onViewDetails }: ProjectListItemProps) {
      ...
      <div className="relative">
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-600 mt-1">Client: <span className="font-medium text-gray-900">{project.clientName}</span></p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4 text-sm">
          ...
          <div className="flex items-center gap-2">
            <Tag className={`w-4 h-4 ${getPriorityColor(project.priority)} flex-shrink-0`} />
            <div>
          ...
          {project.leadAssignee && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-medium text-gray-900 truncate">{project.leadAssignee.name || 'Assigned'}</p>
              </div>
            </div>
          )}
        </div>
        ...
        {project.tags && project.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {project.tags.map((tag, idx) => (
              <span key={idx} className="text-xs px-2.5 py-1 bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-700 rounded-full font-medium">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="flex gap-2 pt-2">
          {project.status === 'pending_assignment' && (
            <button
              ...
              className="flex-1 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white rounded-lg font-medium shadow-md transition-all duration-300 hover:shadow-lg"
            >
              Assign Project
            </button>
          )}
          <button
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-white rounded-lg font-medium shadow-md transition-all duration-300 hover:shadow-lg"
          >
