...
export default function DailyUpdatesReview() {
    ...
    try {
      const response = await fetch(`/api/daily-updates/${selectedUpdate._id}`, {
        ...
        body: JSON.stringify({
          ...
          projectAssignedToSomeoneElse: editableData.projectAssignedToSomeoneElse,
  ...
  return (
    <div className="max-w-7xl mx-auto py-6 px-4">
      <h2 className="text-3xl font-bold text-gray-900 mb-6">Daily Updates Review</h2>
          ...
          <div className="bg-white rounded-lg shadow">
            ...
            <div className="divide-y divide-gray-200 max-h-96 overflow-y-auto">
              {loading ? (
                ...
                <div className="px-6 py-4 text-center text-gray-500">No updates found</div>
              ) : (
                updates.map((update) => (
                  <button
                    key={update._id}
              ...
              <div className="px-6 py-4 space-y-6 max-h-96 overflow-y-auto">
                ...
                <div className="border-b pb-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    {[
                      { key: 'attendedMorningSession', label: 'Attended morning session' },
                      { key: 'cameOnTime', label: 'Came on time' },
                      { key: 'workedOnProject', label: 'Worked on project' },
                      { key: 'askedForNewProject', label: 'Asked for new project' },
                      { key: 'gotCodeCorrected', label: 'Got code corrected' },
                      { key: 'updatedClient', label: 'Updated client' },
                      { key: 'workedOnTrainingTask', label: 'Worked on training' },
                      { key: 'updatedSeniorTeam', label: 'Updated senior team' },
                      { key: 'updatedDailyProgress', label: 'Updated daily progress' },
                      { key: 'plannedNextDayTask', label: 'Planned next day' },
                      { key: 'completedAllTasks', label: 'Completed all tasks' },
                      { key: 'workedOnMultipleProjects', label: 'Multiple projects' }
                    ].map(({ key, label }) => (
                      <div key={key} className="flex items-center p-2 rounded bg-gray-50 hover:bg-gray-100 cursor-pointer">
                        <input
                          type="checkbox"
                        ...
                        />
                        <span className={editableData?.[key] ? 'text-gray-900 font-medium' : 'text-gray-600'}>{label}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {selectedUpdate.tasksForTheDay && (
                  <div className="border-b pb-4">
                    <h4 className="font-medium text-gray-900 mb-2">Tasks for the Day</h4>
                    <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded">{selectedUpdate.tasksForTheDay}</p>
                  </div>
                )}
                ...
                <div className="border-b pb-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    {[
                      { key: 'informedUnableToComplete', label: 'Informed unable to complete' },
                      { key: 'ensuredProjectReassigned', label: 'Project reassigned' },
                      { key: 'ensuredProjectOnTime', label: 'Project on time' },
                      { key: 'informedBeforeBunking', label: 'Informed before bunking' },
                      { key: 'informedBeforeLate', label: 'Informed before late' },
                      { key: 'informedLeavingMeeting', label: 'Informed leaving meeting' },
                      { key: 'freelancerNeeded', label: 'Freelancer needed' },
                      { key: 'ensuredFreelancerHired', label: 'Freelancer hired' },
                      { key: 'addedToWhatsAppGroup', label: 'Added to WhatsApp group' },
                      { key: 'slackGroupCreated', label: 'Slack group created' },
                      { key: 'projectAssignedToSomeoneElse', label: 'Assigned to someone else' },
                      { key: 'projectInPriority', label: 'Project in priority' },
                      { key: 'followedUpWithClient', label: 'Followed up with client' },
                      { key: 'completedAllProjectTasks', label: 'All project tasks done' },
                      { key: 'setTaskDeadlines', label: 'Set task deadlines' },
                      { key: 'recordedLoomVideos', label: 'Recorded Loom videos' },
                      { key: 'organizedLoomVideos', label: 'Organized Loom videos' },
                ...
                {selectedUpdate.supervisor && (
                  <div className="border-b pb-4">
                    ...
                    <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded">{selectedUpdate.supervisor}</p>
                  </div>
                )}

                {/* Hours Worked */}
                <div className="border-b pb-4">
                  <h4 className="font-medium text-gray-900 mb-2">Hours Worked</h4>
                  <div className="flex items-center gap-2">
                    <Clock className="w-5 h-5 text-purple-600" />
                    <span className="text-lg font-semibold text-gray-900">{selectedUpdate.hoursWorked} hours</span>
                  </div>
                </div>
                ...
                {selectedUpdate.additionalNotes && (
                  <div className="border-b pb-4">
                    ...
                    <p className="text-sm text-gray-700 bg-gray-50 p-3 rounded">{selectedUpdate.additionalNotes}</p>
                  </div>
                )}
