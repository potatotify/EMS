...
import { shouldResetRecurringTask } from "@/lib/taskRecurrence";
...
// POST - Fix daily tasks that weren't reset properly
  ...
  try {
    ...
    for (const task of dailyTasks) {
      try {
        ...
        // Check if task should have been reset
        if (isCompleted) {
          // Check if completed task should have been reset
          const lastCompletedAt = taskAny.completedAt || taskAny.tickedAt;
          if (lastCompletedAt) {
            const check = shouldResetRecurringTask(
              "daily",
              lastCompletedAt,
              taskAny.recurringPattern
            );
            shouldReset = check.shouldReset;
            shouldFix = shouldReset;
          }
        } else if (task.status === "pending") {
          // For pending tasks, check if deadline has passed and should reset
          if (taskAny.deadlineTime) {
            ...
            // Check if deadline has passed (yesterday's deadline)
            if (now > deadlineDate) {
              const check = shouldResetRecurringTask(
                "daily",
                deadlineDate,
    ...
    return NextResponse.json({
      success: true,
      message: "Daily tasks reset fix completed",
      stats: {
        tasksFixed: stats.tasksFixed,
        tasksReset: stats.tasksReset,
        subtasksReset: stats.subtasksReset,
        incompleteSubtasksFined: stats.incompleteSubtasksFined,
        errors: stats.errors,
      }
    });
  } catch (error: any) {
    console.error("[API] Error fixing daily tasks reset:", error);
    return NextResponse.json(
      { error: "Failed to fix daily tasks reset", details: error.message },
      { status: 500 }
    );
  }
}
